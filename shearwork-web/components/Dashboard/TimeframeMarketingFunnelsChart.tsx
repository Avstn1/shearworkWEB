...
const timeframeLabel = (timeframe: Timeframe, year: number) => {
  ...
  switch (timeframe) {
    case 'Q1':
      return `Marketing Funnels (Q1 • Jan-Mar ${year})`
    case 'Q2':
      return `Marketing Funnels (Q2 • Apr-Jun ${year})`
    case 'Q3':
      return `Marketing Funnels (Q3 • Jul-Sep ${year})`
    case 'Q4':
      return `Marketing Funnels (Q4 • Oct-Dec ${year})`
  }
}
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { data: clients, error } = await supabase
          .from('acuity_clients')
          .select(
            'client_id, first_name, last_name, first_appt, second_appt, first_source'
          )
          .eq('user_id', barberId)
        ...
        const filteredClients =
          clients?.filter((client) => {
            ...
            const apptMonth = new Date(
              client.first_appt + 'T00:00:00'
            ).getMonth() + 1
        ...
        filteredClients.forEach((client) => {
          ...
          if (
            client.second_appt &&
            isDateInTimeline(client.second_appt, monthsToQuery, year)
          ) {
        ...
        for (const funnel of funnelData) {
          funnel.retention =
            funnel.new_clients > 0
              ? (funnel.new_clients_retained / funnel.new_clients) * 100
              : 0

          const sourceClients = filteredClients.filter(
            (c) => c.first_source === funnel.source
          )
          ...
          if (sourceClients.length > 0) {
            ...
            if (appointments && appointments.length > 0) {
              ...
              sourceClients.forEach((client) => {
                const firstAppt = appointments.find(
                  (appt) =>
              ...
              if (firstAppointmentRevenues.length > 0) {
                const totalRevenue = firstAppointmentRevenues.reduce(
                  (sum, rev) => sum + rev,
                  0
                )
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-[#555]">Loading marketing funnels...</p>
      </div>
    )
  }
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                ...
                stroke="#8a9b90"
                hide
                domain={[0, (dataMax: number) => dataMax / 0.95]}
              />
              <XAxis
                xAxisId="retention"
                type="number"
              ...
              <YAxis type="category" axisLine={false} tick={false} width={0} />
              ...
              >
                <LabelList
                  ...
                  content={(props: any) => {
                    ...
                    const maxNewClients = Math.max(
                      ...filteredData.map((d) => d.new_clients)
                    )
                    const percentage =
                      maxNewClients > 0
                        ? (entry.new_clients / maxNewClients) * 100
                        : 0
                ...
                <LabelList
                  ...
                  content={(props: any) => {
                    ...
                    const maxClients = Math.max(
                      ...filteredData.map((d) =>
                        Math.max(d.new_clients, d.new_clients_retained)
                      )
                    )
                    const percentage =
                      maxClients > 0
                        ? (entry.new_clients_retained / maxClients) * 100
                        : 0
