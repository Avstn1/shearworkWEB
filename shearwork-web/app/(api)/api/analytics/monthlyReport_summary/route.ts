...
export async function POST(req: Request) {
  try {
    ...
    console.log('📤 Invoking monthlyReport_summary edge function with body:', JSON.stringify(body, null, 2));
    ...
    const { data, error } = await supabase.functions.invoke('monthlyReport_summary', {
      body: bodyPayload
    });
    ...
    if (error) {
      ...
      if (error instanceof FunctionsHttpError) {
        ...
        // Generic error
        return NextResponse.json(
          { success: false, error: error.message || 'Failed to invoke monthlyReport_summary' },
          { status: 500 }
        );
      }
    }
