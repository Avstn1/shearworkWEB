...
export async function POST(req: Request) {
  try {
    ...
    console.log('📤 Invoking aggregate_summary edge function with body:', JSON.stringify(body, null, 2));
    ...
    const { data, error } = await supabase.functions.invoke('aggregate_summary', {
      body: bodyPayload
    });
    ...
    if (error) {
      ...
      if (error instanceof FunctionsHttpError) {
        ...
        // Generic error
        return NextResponse.json(
          { success: false, error: error.message || 'Failed to invoke aggregate_summary' },
          { status: 500 }
        );
      }
    }
    ...
    return NextResponse.json({
      success: true,
      data,
      message: 'Aggregate summary generated successfully'
    });

  } catch (err: any) {
    console.error('💥 Aggregate summary API error:', err);
    return NextResponse.json(
