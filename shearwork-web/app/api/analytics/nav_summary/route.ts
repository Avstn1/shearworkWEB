...
export async function POST(req: Request) {
  try {
    ...
    console.log('📤 Invoking nav_summary edge function with body:', JSON.stringify(body, null, 2));
    ...
    const { data, error } = await supabase.functions.invoke('nav_summary', {
      body: bodyPayload
    });
    ...
    if (error) {
      ...
      if (error instanceof FunctionsHttpError) {
        ...
        // Generic error
        return NextResponse.json(
          { success: false, error: error.message || 'Failed to invoke nav_summary' },
          { status: 500 }
        );
      }
    }
    ...
    return NextResponse.json({
      success: true,
      data,
      message: 'Navigation summary generated successfully'
    });

  } catch (err: any) {
    console.error('💥 Navigation summary API error:', err);
    return NextResponse.json(
