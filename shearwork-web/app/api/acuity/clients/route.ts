...
export async function GET(request: Request) {
  ...
  // Use RPC to get accurate client totals (avoids 1000 row limit)
  const { data: clientAggregates, error: clientAggError } = await supabase
    .rpc('get_client_totals', { p_user_id: user.id })

  if (clientAggError) {
    console.error('Error fetching client totals:', clientAggError)
  }
  ...
  console.log('Client totals calculated for', Object.keys(clientTotals).length, 'clients')
