       ...
       ? `<table>
            ...
            <tbody>
              ${services
                ...
                .join('')}
              <tr><td><strong>Gross Total</strong></td><td>${services.reduce((sum:any,s:any)=>sum+(s.bookings||0),0)}</td><td>100%</td><td>$${services.reduce((sum:any,s:any)=>sum+((s.price||0)*(s.bookings || 0)),0).toFixed(2)}</td><td>--</td></tr>
            </tbody>
          </table>
